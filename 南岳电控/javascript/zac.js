...
$(".container").bind("mouseenter mouseleave",function(e) {
  ...
  //企业文化遮罩层效果
$(".cdf_cf li").bind("mouseenter mouseleave",function(e) {
	   var w = $(this).width();
	   var h = $(this).height();
	   var x = (e.pageX - this.offsetLeft - (w / 2)) * (w > h ? (h / w) : 1);
	   var y = (e.pageY - this.offsetTop - (h / 2)) * (h > w ? (w / h) : 1);
	   ...
	   if(e.type == 'mouseenter'){
		  if(direction==0){
			$(this).find('.bj').css({'display':'block','top':-h+'px','left':'0px'}).animate({'top':'0px'});
		  }else if(direction==1){
			$(this).find('.bj').css({'display':'block','left':w+'px','top':'0px'}).animate({'left':'0px'});
		  }else if(direction==2){
			$(this).find('.bj').css({'display':'block','top':h+'px','left':'0px'}).animate({'top':'0px'});
		  }else if(direction==3){
			$(this).find('.bj').css({'display':'block','left':-w+'px','top':'0px'}).animate({'left':'0px'});
		  ...
		  if(direction==0){
			$(this).find('.bj').animate({'top':-h+'px'});
		  }else if(direction==1){
			$(this).find('.bj').animate({'left':w+'px'});
		  }else if(direction==2){
			$(this).find('.bj').animate({'top':h+'px'});
		  }else if(direction==3){
			$(this).find('.bj').animate({'left':-w+'px'});
